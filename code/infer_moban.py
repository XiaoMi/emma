...
SPECIAL_TOKENS = ["[CLS]", "[SEP]", "[PAD]", "[speaker1]", "[speaker2]", '[sad]', '[None]', '[True]', '[False]', '[Keywords]']
...
def build_input_from_segments(history, reply, tokenizer, with_eos=True):
    ...
    bos, eos, pad, speaker1, speaker2 = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[:5])
    # print("bos, eos, pad, speaker1, speaker2:", bos, eos, pad, speaker1, speaker2 )
...
def sample_sequence(history, tokenizer, model, args, current_output=None):
    special_tokens_ids = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[:5])
...
def main():
    ...
    parser.add_argument("--datapath", type=str, default="data/test_all.json", help="Path of the dataset.")
    parser.add_argument("--out_path", type=str, default="test_dialog", help="Path of response generated.")
    ...
    for instance in tqdm(dataset, mininterval=1):
        ...
        history = tokenize(query_text.split("|"))
        with torch.no_grad():
            out_ids = sample_sequence(history, tokenizer, model, args)
        out_text = tokenizer.decode(out_ids, skip_special_tokens=False)
        # print("***************************************")
        out_text = out_text.replace(" ","")
        for token in ["[CLS]", "[SEP]", "[PAD]", "[speaker1]", "[speaker2]", '[sad]', '[None]', '[True]', '[False]']:
            out_text = out_text.replace(token, "")
        predictions.append(instance[0].replace(" ","") +"\t" + instance[1].replace(" ","") + "\t" + out_text)
